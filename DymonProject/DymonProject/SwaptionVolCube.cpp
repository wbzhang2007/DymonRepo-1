...
double SwaptionVolCube::getVol(double strike, double maturity, double tenor){
	AbstractCurve<double>* volCurveAlongStrike = getVolCurveAlongStrike(maturity,tenor);
	return volCurveAlongStrike->getValue(strike);
}

AbstractCurve<double>* SwaptionVolCube::getVolCurveAlongStrike(double maturity, double tenor){
	...
	for (CubeIterator i = _cube->begin(), j=_cube->begin()++; j!=_cube->end(); i++,j++){
		...
		double startVol = lowerStrikeSurface->getValue(maturity,tenor);
		double endVol = upperStrikeSurface->getValue(maturity,tenor);
